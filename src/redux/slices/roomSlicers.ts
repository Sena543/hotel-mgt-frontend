...
import { arrayUnion, doc, updateDoc, getDocs, collection, QuerySnapshot, DocumentData } from "firebase/firestore/lite";
	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchAllRooms.fulfilled, (state, action) => {
			...
			guestsData.forEach((guest) => {
				if (availableRooms.includes(guest.roomAssigned) && dayjs().isBefore(dayjs(guest.checkOut))) {
