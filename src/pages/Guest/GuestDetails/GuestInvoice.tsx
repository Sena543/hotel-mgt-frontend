...
function GuestInvoice({ open, setOpen, guestDetails }: GuestInvoice) {
	...
	const otherCharges = () => {
		...
		guestMeals.forEach((meal) => {
			...
			if (meal.mealID) {
				mealObj["type"] = "Meal";
				beverageObj["price"] = meal.mealPrice;
	...
	const sum = formatForTable().map(
		(row) =>
			row.pricePerNight *
			Number(dayjs(formattedDate(guestDetails.checkOut)).diff(formattedDate(guestDetails.checkIn), "day"))
	)[0];
			...
			<div className="invoiceContent">
				<div className="invoice-header">
					...
					<div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
						...
						<Typography style={{ ...styles.headerStyle, fontSize: "12px" }}>City</Typography>
						<Typography style={{ ...styles.headerStyle, fontSize: "12px" }}>Country</Typography>

						<Typography style={{ ...styles.headerStyle, fontSize: "12px" }}>Postal Address</Typography>
					</div>
				</div>
				<div className="invoice-guest-details">
					<div className="inv-guest-dets">
						...
						<Typography style={{ fontSize: "15px" }}>{guestDetails?.contact}</Typography>
					</div>
					<div className="inv-dets">
						<Typography style={{ fontWeight: "bold", fontSize: "15px" }}>Date</Typography>
						<Typography style={{ fontSize: "15px" }}>
							{new Intl.DateTimeFormat("en-GB").format(new Date())}
						</Typography>
						<Typography style={{ fontSize: "15px" }}>Check in date: {guestDetails?.checkIn}</Typography>
						<Typography style={{ fontSize: "15px" }}>Check out date: {guestDetails?.checkOut}</Typography>
					</div>
				</div>
